-- 015_ungate_sectors — per-sector gating flag (Data Analyst on-ramp).
-- Adds sectors.is_gated; ungated sectors skip the level-lock entirely (the same
-- exemption shape as the daily challenge). The Data Analyst sector seeds as
-- is_gated=false via content:seed. submit_attempt v4 carries 014 verbatim except
-- the gating block now also reads the sector flag. Everything else (badges,
-- streak-freeze, daily-goal, award_badges, merge_local_progress) is unchanged.

alter table public.sectors
  add column is_gated boolean not null default true;
...
begin
  ...
  select coalesce(c.unlock_level_override, ur.min_level), s.is_gated
    into v_unlock_level, v_sector_gated
  ...
  -- Ungated sectors (the Data Analyst on-ramp) and the daily featured challenge
  -- are exempt from level-gating.
  if v_user_level < coalesce(v_unlock_level, 1) and coalesce(v_sector_gated, true) then
